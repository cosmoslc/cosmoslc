...
async function run() {
  const { data } = await supabase.from('students').select('id, name, balance').ilike('name', '%abdulloh rustamov%');
  console.log('Abdulloh:', data);

  const { data: allDebtors } = await supabase.from('students').select('id, name, balance').lt('balance', 0);
  console.log('Debtors (<0):', allDebtors?.length, allDebtors);
}
run();
