...
function renderNavIcon(icon, size = 18, id) {
  const ResolvedIcon = icon || (id ? FALLBACK_NAV_ICONS[id] : null);
...
export function AppShell({
  ...
  const coreIds = ["dashboard", "home", "groups", "schedule"];
  const learningIds = ["students", "tasks", "rating", "dictionary", "games"];
  const accountIds = ["analytics", "payments", "profile"];
                ...
                {coreItems.map((item) => {
                  ...
                  return (
                    <li key={item.id}>
                      ...
                      >
                        <span className="ic">
                          {renderNavIcon(item.icon, 16, item.id)}
                        </span>
                        {!collapsed && (
