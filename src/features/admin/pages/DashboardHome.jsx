...
export function DashboardHome({
  ...
  const paymentTotal = rawPayments
    .filter((p) => isSelectedPeriod(p.date))
  ...
  const teacherPaymentsTotal = (directorData?.teacherPayments || [])
    .filter((tp) => matchesMonthKey(tp.month))
  ...
  const filteredLeads = useMemo(() => {
    return rawLeads.filter((l) => {
      ...
      if (!isSelectedPeriod(l.createdAt)) return false;
      return true;
    });
  }, [rawLeads, selectedBranch, selectedYear, selectedMonth]);
