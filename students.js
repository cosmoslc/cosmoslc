...
function parseGroupMemberships(s) {
  const raw = s.group_memberships ?? s.groupMemberships;
  if (!raw) return {};
...
function fromRow(s) {
  return {
    ...
    groupMemberships: parseGroupMemberships(s),
...
export async function addStudent(payload) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      console.warn("Supabase addStudent inserting without balance/group_memberships column:", error.message);
      const { balance, group_memberships, ...rest } = base;
      ...
      if (e2) {
        ...
        if (e3) throw e3;
        return { ...fromRow(d3), balance: payload.balance || 0, groupMemberships: payload.groupMemberships || {} };
      }
      return { ...fromRow(d2), balance: payload.balance || 0, groupMemberships: payload.groupMemberships || {} };
    }
    return fromRow(data);
  } catch (err) {
...
export async function updateStudent(id, payload) {
  ...
  if (payload.groupMemberships !== undefined) {
    patch.group_memberships = payload.groupMemberships || {};
  }
  ...
  if (error) {
    console.warn("Supabase updateStudent error, trying fallback without balance/group_memberships/format:", error.message);
    const retryPatch = { ...patch };
    if (retryPatch.balance !== undefined) delete retryPatch.balance;
    if (retryPatch.group_memberships !== undefined) delete retryPatch.group_memberships;
