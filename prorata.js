...
export function calculateProratedFee({
  ...
  // If student missed 1 or 2 lessons (or 0) AND freeAllowance > 0, they pay full month fee
  if (freeAllowance > 0 && missedLessons <= freeAllowance) {
    return {
      calculatedFee: fee,
      isProrated: false,
      totalLessons,
  ...
  // Student joined after lessons passed:
...
export function calculateStudentGroupFee({
  ...
  // If student profile itself is paused ("Muzlatilgan"):
  ...
  if (isTrial) {
    return {
      ...
      activationDate: null,
      allLessonDates,
      reason: "Sinov darsida (Faollashtirilgach to'lov hisoblanadi)",
    };
  }
  ...
  if (!actDateStr || (actMonth && actMonth < activeMonth)) {
    // Activated before this month -> Full month
    return {
      status: "active",
      statusLabel: "Faol",
      isTrial: false,
  ...
  if (actMonth && actMonth > activeMonth) {
    // Activated in future month
    return {
      status: "active",
      statusLabel: "Faol",
      isTrial: false,
      ...
      activationDate: actDateStr,
      allLessonDates,
      reason: "O'quvchi bu oydan keyin faollashtiriladi",
    };
  }
